...
describe Vote, type: :model do
  it "should be associated with a proposal" do
    vote = Vote.new
    vote.vote = 'agree'
    vote.user = FactoryBot.create(:user)
    expect(vote.valid?).to eq(false)

    vote.proposal = FactoryBot.create(:proposal)
  ...
  it "can't vote twice on the same proposal by the same user" do
    ...
    @vote2 = FactoryBot.build(:vote, proposal: @vote1.proposal, user: @vote1.user)
    expect(@vote2.valid?).to eq(false)
  end
  it "can vote on the same proposal with 2 people" do
    @vote1 = FactoryBot.create(:vote)
    @vote2 = FactoryBot.build(:vote, proposal: @vote1.proposal) # new user
