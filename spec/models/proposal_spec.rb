...
describe Proposal, type: :model do
  it "should have an associated user" do
    prop = Proposal.new
    prop.committee = FactoryBot.create(:committee)
    prop.title = "Hello People"
    prop.status = 'Submitted'
    expect(prop.valid?).to eq(false)

    prop.owner = FactoryBot.create(:user)
  ...
  it "orders the votes by created_at date" do
    proposal = FactoryBot.create(:proposal)
    vote2 = FactoryBot.create(:vote, proposal: proposal, created_at: 1.second.ago)
    vote1 = FactoryBot.create(:vote, proposal: proposal, created_at: 2.seconds.ago)
  ...
  it "should provide all revisions in descending order" do
    prop = FactoryBot.create(:proposal)
    rev1 = FactoryBot.create(:revision, proposal: prop)
  ...
  describe "when checking the 'status_string'" do
    it "should print only the status for a Submitted proposal" do
      prop = FactoryBot.create(:proposal, status: 'Submitted')
  ...
  describe "with votes" do
    before(:each) do
      @prop = FactoryBot.create(:proposal)
      @rev1 = FactoryBot.create(:revision, proposal: @prop)
      @vote = FactoryBot.create(:vote, proposal: @prop, vote: 'agree', comment: '')

      @cm = FactoryBot.create(:committee_member, user: @vote.user, committee: @prop.committee)
    end
    ...
    describe "if there is a 2nd committee_members who hasn't voted" do
      before(:each) do
        @cm2 = FactoryBot.create(:committee_member, committee: @prop.committee)
      end
      ...
      describe "when the 2nd member votes disagree" do
        before(:each) do
          @vote = FactoryBot.create(:vote, user: @cm2.user, proposal: @prop, vote: 'disagree', comment: 'hi')
        end
      ...
      describe "if there is a 3rd committee_members who hasn't voted" do
        before(:each) do
          FactoryBot.create(:committee_member, committee: @prop.committee)
        end
  ...
  describe "with existing Review+Submitted proposals" do
    before(:each) do
      ...
      @p1 = FactoryBot.create(:proposal, status: 'Review', review_end_date: Date.current.prev_day(1))
      @r1 = FactoryBot.create(:revision, proposal: @p1)

      # submitted proposal which will not be advanced
      @p2 = FactoryBot.create(:proposal, status: 'Submitted')
      @r2 = FactoryBot.create(:revision, proposal: @p2)

      # review proposal which will NOT be advanced (due to date)
      @p3 = FactoryBot.create(:proposal, status: 'Review', review_end_date: Date.current)
      @r3 = FactoryBot.create(:revision, proposal: @p3)
  ...
  describe "with 'Voting' proposals" do
    before(:each) do
      ...
      @p4 = FactoryBot.create(:proposal, status: 'Voting', vote_end_date: Date.current)
      @r4 = FactoryBot.create(:revision, proposal: @p4)

      # voting proposal which will be failed (due to no votes)
      @p5 = FactoryBot.create(:proposal, status: 'Voting', vote_end_date: Date.current.prev_day(1))
      @r5 = FactoryBot.create(:revision, proposal: @p5)

      FactoryBot.create(:committee_member, committee: @p4.committee)
      FactoryBot.create(:committee_member, committee: @p5.committee)
    end
    ...
    describe "calling update_proposal_states when all of the voting-members have voted agree" do
      before(:each) do
        @p5.committee.committee_members.each do |cm|
          FactoryBot.create(:vote, proposal: @p5, user: cm.user, vote: 'agree')
        end
        ActionMailer::Base.deliveries.clear
        Proposal.update_proposal_states
      end
    ...
    describe "calling update_states when all voting-members have voted disagree, but the time has not completed" do
      before(:each) do
        @p4.committee.committee_members.each do |cm|
          FactoryBot.create(:vote, proposal: @p4, user: cm.user, vote: 'disagree')
        end
        ActionMailer::Base.deliveries.clear
        Proposal.update_proposal_states
      end
